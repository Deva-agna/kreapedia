    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('berita.update') }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <input type="hidden" name="berita_slug" value="{{ $berita->slug }}">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="judul">Judul</label>
                            <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul" name="judul" placeholder="Masukan judul . . ." value="{{ old('judul', $berita->judul) }}">
                            @error('judul')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="select2-icons form-control" id="select2-icons" name="kategori">
                                ...
                                <option value="{{ $kategori->id }}" data-icon="{{ $kategori->icon }}" {{old('kategori', $berita->kategori_id) == $kategori->id ? 'selected' : ''}}>{{ $kategori->kategori }}</option>
                                @endforeach
                            </select>
                            @error('kategori')
                    ...
                    <div class="col-12">
                        ...
                        <input type="hidden" name="konten" value="{{ old('konten', $berita->konten) }}" id="konten">
                    </div>
                        ...
                        <div class="border rounded p-2">
                            ...
                            <div class="media flex-column flex-md-row">
                                <div class="thumbnail @error('gambar') is-error-border @enderror">
                                    @if($berita->gambar)
                                    <img src="{{ asset('asset-berita/'.$berita->gambar) }}" id="blog-feature-image" class="preview-image" onerror="this.style.display='none'">
                                    @endif
                                </div>
                                <div class=" flex-column">
                                    ...
                                    <div class="form-group mb-0">
                                        <div class="custom-file">
                                            ...
                                            <input type="hidden" name="gambar_old" value="{{$berita->gambar}}">
                                        </div>
                                    </div>
                                    @error('gambar')
                                    <div class="message-error">
...
<script>
    $('#gambar').on('change', function() {
        ...
        if (gambar.files.length > 0) {
            ...
            gambar.value = "";
            previewImage.src = "{{asset('asset-berita/'. $berita->gambar)}}";
